...
class PhotosController < BaseController
  ...
  def fb_album
    flash[:notice] = "fb_album enter #{params}"
    ...
    access_token = current_user.fb_access_token if access_token == nil
    #fb_user_id = params[:fb_user_id]
    #begin
        @graph = Koala::Facebook::API.new(access_token)
        if @graph == nil
          raise 'Graph is nill acccess_token:' + access_token
        end
    ...
    if request.xhr?
      if(id != nil)
        @album = @graph.get_object(id, :fields => "cover_photo,name,id,type,photos.fields(id,picture)")
      end
      render :partial => "photos/fb_album"
    else
        @albums = @graph.get_connections("me","albums", :fields => "cover_photo,name,id,type,photos.fields(id,picture)")
        if(id != nil)
          @album = @graph.get_object(id, :fields => "cover_photo,name,id,type,photos.fields(id,picture)")
        end
    end
